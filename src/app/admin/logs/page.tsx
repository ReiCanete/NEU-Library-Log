...
const VisitHistory = ({ studentId, currentVisitId }: { studentId: string, currentVisitId: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        console.error("Visit history error:", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="py-4 flex gap-2"><Loader2 className="h-4 w-4 animate-spin text-[#c9a227]" /> <span className="text-xs text-slate-400">Loading history...</span></div>;
  if (history.length === 0) return <p className="text-sm text-slate-400 py-4">No historical records found.</p>;
  
  return (
    <div className="space-y-2 mt-2">
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{history.length} recorded visit(s)</p>
      {history.slice(0, 5).map(visit => (
        <div key={visit.id} className={`text-sm p-3 rounded-xl border transition-all ${visit.id === currentVisitId ? 'bg-[#c9a227]/10 border-[#c9a227]/30' : 'bg-white border-slate-100'}`}>
          ...
          {visit.id === currentVisitId && <span className="text-[9px] text-[#c9a227] font-black uppercase tracking-tighter mt-1 block">Selected Visit</span>}
        </div>
      ))}
    </div>
  );
...
export default function VisitorLogs() {
  ...
  const filteredVisits = useMemo(() => {
    ...
    return allVisits.filter(v => {
      ...
      else if (typeFilter === 'employee') matchesType = ['Faculty', 'Administrative Staff', 'Library Staff'].includes(visitType);
      else matchesType = visitType === typeFilter;
  ...
  return (
    <AdminLayout>
        ...
        <Card className="rounded-2xl border-[#d4e4d8] shadow-sm bg-white p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-[#4a6741]">Search Name or ID</Label>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300" />
                <Input placeholder="Find visitor..." className="h-11 pl-10 rounded-xl bg-[#f0f4f1] border-none font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-[#4a6741]">Role Filter</Label>
            ...
            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-[#4a6741]">Visit Purpose</Label>
          ...
          <Table>
            <TableHeader className="bg-[#1a3a2a]">
              <TableRow className="border-none hover:bg-[#1a3a2a]">
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold h-12 px-6">Visitor ID</TableHead>
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold">Full Name</TableHead>
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold text-center">Role / Type</TableHead>
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold">Purpose</TableHead>
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold">Timestamp</TableHead>
                <TableHead className="text-white text-[10px] tracking-widest uppercase font-bold text-right px-6">Access</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {visitsLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="h-96 text-center text-slate-400 font-medium">No activity matching your filters.</TableCell></TableRow>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <Badge className={`${TYPE_STYLES[visitType] || 'bg-slate-100 text-slate-600'} text-[8px] font-black uppercase px-2 py-0.5 border`}>
                        {visitType}
                      </Badge>
                    </TableCell>
                    <TableCell>
          ...
          <>
            <div className="bg-[#1a3a2a] p-6 flex items-center justify-between text-white shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-black uppercase tracking-widest text-sm">Visitor Detail Profile</h2>
              </div>
              ...
              <div className="flex flex-col items-center text-center gap-4 py-4">
                <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-[#1a3a2a] to-[#0a2a1a] flex items-center justify-center shadow-xl relative group">
                  ...
                  <div className="absolute -bottom-2 -right-2 bg-white p-2 rounded-xl shadow-lg border border-gray-100">
                    <Badge className={`${TYPE_STYLES[selectedVisit.visitorType || 'Student']} text-[8px] font-black uppercase px-2 py-0.5 border`}>
              ...
              <div className="space-y-4">
                <div className="bg-[#f0f7f2] rounded-2xl p-5 border border-[#d4e4d8]/30">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-black text-[#4a6741] uppercase tracking-widest">Current Session</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Purpose</p>
                      <p className="text-xs font-bold text-[#1a3a2a]">{selectedVisit.purpose}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Login Method</p>
                      <p className="text-xs font-bold text-[#1a3a2a]">{selectedVisit.loginMethod || 'Card Scan'}</p>
                    </div>
                    <div className="col-span-2 pt-2 border-t border-slate-100">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Institutional Unit</p>
                      <p className="text-xs font-bold text-[#1a3a2a]">{selectedVisit.college || 'General'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#f0f7f2] rounded-2xl p-5 border border-[#d4e4d8]/30">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-black text-[#4a6741] uppercase tracking-widest">Timestamp</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="space-y-1">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Date</p>
                ...
                <div className="bg-[#f0f7f2] rounded-2xl p-5 border border-[#d4e4d8]/30">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-black text-[#4a6741] uppercase tracking-widest">History Log (Last 10)</span>
                  </div>
      ...
      {sidePanelOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-[90] animate-in fade-in duration-300" onClick={() => setSidePanelOpen(false)} />
      )}
    </AdminLayout>
  );
}
