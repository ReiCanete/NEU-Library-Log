...
function LoginContent() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-6 neu-dark-bg">
        <Loader2 className="h-12 w-12 animate-spin text-[#c9a227]" />
        <p className="text-[#c9a227] text-lg font-black tracking-widest animate-pulse">VERIFYING STAFF ACCESS...</p>
      </div>
    );
  }
...
export default function AdminLogin() {
  return (
    <Suspense fallback={<div className="min-h-screen neu-dark-bg flex items-center justify-center"><Loader2 className="animate-spin text-[#c9a227]" /></div>}>
      <LoginContent />
    </Suspense>
  );
}