...
import { Download, Calendar, FileText, Loader2, TrendingUp, Users, Share2, Info, LayoutList, ShieldAlert } from 'lucide-react';
      ...
      <div className="space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <Button variant="outline" onClick={exportCSV} disabled={filteredData.length === 0} className="flex-1 md:flex-none h-16 px-8 rounded-[1.5rem] border-[#d4e4d8] text-[#1a3a2a] font-black flex gap-2 shadow-sm transition-all hover:bg-slate-50">
              <Share2 className="h-5 w-5" /> Export CSV
            </Button>
            <Button onClick={generatePDF} disabled={isGenerating || !stats} className="flex-1 md:flex-none bg-[#c9a227] text-[#0a2a1a] hover:bg-[#b08d20] rounded-[1.5rem] px-10 h-16 font-black shadow-xl flex gap-3 transition-all">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-end">
              <div className="flex-1 space-y-3">
                ...
                <div className="relative">
                  ...
                  <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="h-14 pl-12 bg-[#f0f4f1] border-none rounded-2xl font-black text-[#1a3a2a]" />
                </div>
              </div>
              <div className="flex-1 space-y-3">
                ...
                <div className="relative">
                  <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-300" />
                  <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="h-14 pl-12 bg-[#f0f4f1] border-none rounded-2xl font-black text-[#1a3a2a]" />
                </div>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-10">
            {visitsLoading ? <div className="h-64 flex items-center justify-center"><Loader2 className="animate-spin text-[#c9a227] h-12 w-12" /></div> : !stats ? 
              ...
              <div className="space-y-12 animate-in fade-in duration-500">
                ...
                <div className="space-y-6">
                   <h4 className="text-sm font-black text-[#1a3a2a] uppercase tracking-widest flex gap-2 items-center"><ShieldAlert className="h-5 w-5 text-red-600" /> Restricted During Period</h4>
                   {filteredBlocked.length === 0 ? (
                     <div className="p-10 border-2 border-dashed border-[#d4e4d8] rounded-[2rem] text-center text-[#4a6741]/40 font-black uppercase text-xs">No restrictions issued during this timeframe</div>
                   ) : (
                     <div className="rounded-[1.5rem] border border-red-100 overflow-hidden">
                       <Table>
                         <TableHeader className="bg-red-50">
                           <TableRow className="border-none">
                             <TableHead className="font-black text-[9px] text-red-900 h-10 uppercase">Name</TableHead>
                             <TableHead className="font-black text-[9px] text-red-900 h-10 uppercase">Reason</TableHead>
                             <TableHead className="font-black text-[9px] text-red-900 h-10 uppercase text-right">Date</TableHead>
                           </TableRow>
                         </TableHeader>
                         <TableBody>
                           {filteredBlocked.map(b => (
                             <TableRow key={b.id} className="bg-red-50/10">
                               <TableCell className="font-black text-xs text-red-900">{b.fullName}</TableCell>
                               <TableCell className="font-bold text-xs text-[#4a6741]">{b.reason}</TableCell>
                               <TableCell className="text-right text-[10px] font-black text-red-900/40 uppercase tabular-nums">{format(b.blockedAt.toDate(), 'PP')}</TableCell>
                             </TableRow>
                           ))}
                         </TableBody>
                       </Table>
