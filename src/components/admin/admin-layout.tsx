...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (authLoading || checkingRole || isAdmin === null || !currentTime) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-[#0a2a1a] overflow-hidden">
  ...
  return (
    <SidebarProvider style={{ "--sidebar-width": "280px" } as React.CSSProperties}>
      <div className="flex min-h-screen bg-[#f0f4f1] w-full font-body overflow-hidden">
        <Sidebar className="border-r border-[#c9a227]/30 bg-[#0a2a1a] text-white">
          <SidebarHeader className="p-8 border-b border-[#c9a227]/10 flex flex-col items-center gap-4">
            ...
            />
            <div className="text-center pb-2 border-b-2 border-[#c9a227] w-full">
        ...
        <SidebarInset className="flex flex-col flex-1 overflow-hidden">
          <header className="h-20 bg-white border-b border-[#d4e4d8] flex items-center justify-between px-10 sticky top-0 z-50 shadow-sm">
            ...
            <div className="flex items-center gap-6">
              <div className="text-right hidden md:block">
                <p className="text-sm font-black text-[#1a3a2a]">{user?.displayName || 'Authorized Admin'}</p>
          ...
          <main className="p-10 flex-1 animate-in fade-in duration-700 bg-[#f0f4f1] overflow-y-auto">
            {children}
          </main>
        </SidebarInset>
          ...
          <AlertDialogContent className="rounded-[2.5rem] border-none shadow-2xl p-10 max-w-md">
            <AlertDialogHeader className="space-y-4">
              ...
              <AlertDialogDescription className="text-[#4a6741] font-medium text-base">
                Are you sure you want to logout? You will need to re-authenticate to access system logs.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="mt-8 gap-4">
              <AlertDialogCancel className="rounded-2xl h-14 px-8 font-black border-[#d4e4d8] text-[#4a6741] hover:bg-[#f0f4f1]">Cancel</AlertDialogCancel>
              <AlertDialogAction 
                onClick={handleLogout}
                className="rounded-2xl h-14 px-10 font-black bg-red-600 text-white hover:bg-red-700 shadow-xl shadow-red-100"
              >
                Logout Now
              </AlertDialogAction>
            </AlertDialogFooter>
